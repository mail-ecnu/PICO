...
class State(object):
    ...
    '''
    def __init__(self, world0, goals, diagonal, num_agents=1, Te=2):
        ...
        self.Te = Te
        self.priorities = [0 for i in range(num_agents)]
        init_message = [0 for i in range(512)]
        self.messages = [init_message for i in range(num_agents)]
        self.aggregate_messages = [init_message for i in range(num_agents)]
        self.message_buffers = [[] for i in range(num_agents)]
        self.waiting_times = [ 0 for i in range(num_agents)]
        # low:0 high:1 undefined:-1
        self.agent_level = np.zeros(num_agents)
    ...
    def getPriority(self, agent_id):
        return self.priorities[agent_id-1]

    def setPriority(self, agent_id, priority):
        self.priorities[agent_id-1] = priority

    def getMessage(self, agent_id):
        return self.messages[agent_id-1]

    def setMessage(self, agent_id, message):
        self.messages[agent_id-1] = message

    def getAggregateMessage(self, agent_id):
        return self.aggregate_messages[agent_id-1]

    def setAggregateMessage(self, agent_id, message):
        self.aggregate_messages[agent_id-1] = message

    def getMessageBuffer(self, agent_id):
        return self.message_buffers[agent_id-1]

    def resetMessgeBuffer(self, agent_id):
        self.message_buffers[agent_id-1] = []

    def send2MessgeBuffer(self, agent_id, message):
        self.message_buffers[agent_id-1].append(message)

    def resetWaitingTime(self, agent_id):
        self.waiting_times[agent_id-1] = 0

    def getWaitingTime(self, agent_id):
        return self.waiting_times[agent_id-1]

    def notReceiveLargerWeight(self,agent_id):
        wt = self.waiting_times[agent_id-1]
        
        if wt == -1:
            self.resetWaitingTime(agent_id)
            self.appendHighLevel(agent_id)
        
        self.waiting_times[agent_id-1] -= 1
        

    def waitForHighLevelSignal(self, agent_id, visible_agents):
        if self.getWaitingTime(agent_id) < 0:
            self.resetWaitingTime(agent_id)
        
        for va in visible_agents:
            if self.getAgentLevel(va) == 1:
                self.resetWaitingTime(agent_id)
                self.appendLowLevel(agent_id)
                return 
    ...
    def getAgentLevel(self, agent_id):
        return self.agent_level[agent_id-1]
        
    def appendHighLevel(self, agent_id):
        self.agent_level[agent_id-1] = 1

    def appendLowLevel(self, agent_id):
        self.agent_level[agent_id-1] = 0

    def appendUndefined(self, agent_id):
        self.agent_level[agent_id-1] = -1
...
#        def eq(f1,f2):return abs(f1-f2)<0.001
    ...
    def moveAgent(self, direction, agent_id):
        ...
        # No collision: we can carry out the action(allow leaving goal)
        self.state[ax,ay] = 0
...
class MAPFEnv(gym.Env):
    ...
    # Initialize env
    def __init__(self, num_agents=1, observation_size=11,world0=None, goals0=None, DIAGONAL_MOVEMENT=False, SIZE=(10,40), PROB=(0,.5), 
                FULL_HELP=False,blank_world=False, env_fix=False, regular_env=False, Te=2):
        """
        Args:
    ...
    def getMetrics(self):
        return self.total_move, self.collision_total, self.collision_agent, self.collision_static

    def getCommMetrics(self):
        return self.world.agent_level
    ...
    def _setWorld(self, world0=None, goals0=None,blank_world=False):
        ...
        if not (world0 is None) and not self.env_fix:
        ...
        if not self.env_fix:
            ...
            world = (world0==-1).astype(int) if not (world0 is None) else self.getObstacleMap()
    ...
    def getVisibleAgents(self, agent_id, d):
        # assert(agent_id>0)
        top_left=(self.world.getPos(agent_id)[0]-self.observation_size//2,self.world.getPos(agent_id)[1]-self.observation_size//2)
        bottom_right=(top_left[0]+self.observation_size,top_left[1]+self.observation_size)        

        # including current agent
        visible_agents = []
        for i in range(top_left[0],top_left[0]+self.observation_size):
            for j in range(top_left[1],top_left[1]+self.observation_size):
                if i>=self.world.state.shape[0] or i<0 or j>=self.world.state.shape[1] or j<0:
                ...
                if self.world.state[i,j]>0:
                    current_agent_id = self.world.state[i,j]
                    visible_agents.append(current_agent_id)

        return visible_agents

    def checkHighLevel(self, agent_id,visible_agents):
        own_priority = self.world.getPriority(agent_id)
        for va in visible_agents:
            if own_priority < self.world.getPriority(va):
                self.world.appendLowLevel(agent_id)
        
    def checkLowLevel(self, agent_id,visible_agents):
        for va in visible_agents:
            if self.world.getAgentLevel(va) == 1:
                return

        self.world.appendUndefined(agent_id)

    def checkUndefinedLevel(self, agent_id,visible_agents):
        own_priority = self.world.getPriority(agent_id)

        for va in visible_agents:
            if own_priority < self.world.getPriority(va):
                self.world.waitForHighLevelSignal(agent_id,visible_agents)

        self.world.notReceiveLargerWeight(agent_id)
            
    def aggregateAndBoardcast(self,agent_id,visible_agents):
        # assert(agent_id>0)

        # including current agent
        visible_agents_messages=[self.world.getMessage(va) for va in visible_agents]

        agm = np.mean(np.array(visible_agents_messages),axis=0)


        for va in visible_agents:
            self.world.send2MessgeBuffer(va, agm)
    
    def aggregateAndBoardcastByPriorityBit(self,agent_id):
        # assert(agent_id>0)
        top_left=(self.world.getPos(agent_id)[0]-self.observation_size//2,self.world.getPos(agent_id)[1]-self.observation_size//2)
        bottom_right=(top_left[0]+self.observation_size,top_left[1]+self.observation_size)        

        # including current agent
        visible_agents_messages=[]
        visible_agents = []
        for i in range(top_left[0],top_left[0]+self.observation_size):
            for j in range(top_left[1],top_left[1]+self.observation_size):
                if i>=self.world.state.shape[0] or i<0 or j>=self.world.state.shape[1] or j<0:
                ...
                if self.world.state[i,j]>0:
                    current_agent_id = self.world.state[i,j]
                    visible_agents.append(current_agent_id)
                    visible_agents_messages.append(self.world.getMessage(current_agent_id))
        # import pdb; pdb.set_trace()

        agm = np.mean(np.array(visible_agents_messages),axis=0)

        # self.world.setAggregateMessage(agent_id, agm)

        for va in visible_agents:
            if self.world.getAgentLevel(agent_id)==1:
                continue
            self.world.send2MessgeBuffer(va, agm)

    def aggregateAllNeighbor(self,agent_id):
        # assert(agent_id>0)
        top_left=(self.world.getPos(agent_id)[0]-self.observation_size//2,self.world.getPos(agent_id)[1]-self.observation_size//2)
        bottom_right=(top_left[0]+self.observation_size,top_left[1]+self.observation_size)        
    ...
    def reduceMessageBuffer(self, agent_id):
        # assert(agent_id>0)
        messages = self.world.getMessageBuffer(agent_id)
        if len(messages) == 0:
            reduce_message = self.world.getMessage(agent_id)
        else:
            reduce_message = np.mean(np.array(messages),axis=0)
        self.world.setAggregateMessage(agent_id, reduce_message)
    ...
    def _step(self, action_input,episode=0,output_priority=False):
        ...
        # agentGoalLocation = self.world.getGoal(agent_id)
        ...
        if action==0:#staying still
            ...
            if (action_status == 1): # reached goal
                ...
                # dxs=np.abs(agentGoalLocation[0]-agentStartLocation[0])
                # dys=np.abs(agentGoalLocation[1]-agentStartLocation[1])
                # mags=dxs+dys
                # dxe=np.abs(agentGoalLocation[0]-agentEndLocation[0])
                # dye=np.abs(agentGoalLocation[1]-agentEndLocation[1])
                # mage=dxe+dye
                # if mage < mags:
                #     reward = NEAR_COST
                # elif mage == mags:
        ...
        self.mutex.release()
        if output_priority:
            priority = self.world.getPriority(agent_id)
            return state, reward, done, nextActions, on_goal, blocking, valid_action, priority
